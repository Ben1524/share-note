...
#define MYTINYSTL_VECTOR_H_   // 条件编译头文件命名方法
...
#undef max  // 防止和 std::max 冲突,取消宏定义
#endif // max

#ifdef min
#pragma message("#undefing marco min") //message()函数用于在编译时输出一条消息
#undef min
#endif // min
...
class vector
{
  ...
  // static_assert() 函数用于在编译时判断一个条件是否为真，如果为假则终止编译并输出错误信息
  // std::is_same<T, bool>::value 为真时，终止编译并输出错误信息,否则继续编译
  // std::is_same是一个类型判断函数，判断两个类型是否相同，相同返回true，否则返回false,这里判断T是否为bool类型
  // 这里的value是一个静态成员变量，用于判断T是否为bool类型
  //如果是bool类型，终止编译并输出错误信息
public:
  // vector 的嵌套型别定义
  typedef mystl::allocator<T>                      allocator_type;
  typedef mystl::allocator<T>                      data_allocator;

 //typename限定符，告诉编译器allocator_type是一个类型，而不是一个成员变量
...
public:
  // 构造、复制、移动、析构函数
  vector() noexcept //noexcept关键字用于指定函数不抛出异常,这里指定vector()函数不抛出异常
  // noexcept关键字只能用于函数声明，不能用于函数定义
  //作用：告诉编译器，函数不会抛出异常，这样编译器可以对函数进行优化
  { try_init(); }

  // explicit 关键字用于禁止隐式转换,必须使用显示转换
  explicit vector(size_type n)
  { fill_init(n, value_type()); }

  vector(size_type n, const value_type& value)
  { fill_init(n, value); } // fill_init用于初始化,value为初始化值

  // std::enable_if<condition, type>::type用于条件编译
    // condition为true时，type为int，否则为void
    // 这里的作用是，当Iter为输入迭代器时，才能调用该构造函数
...
// 移动构造函数，右值引用，同时把rhs的成员变量置为空不允许rhs再次使用
 ...
 // std::initializer_list<T>是一个模板类，用于初始化列表
 // 好处：可以用于初始化vector，map，set等容器
// 同时支持使用{}进行初始化
...
// 重载=，并且使用了交换拷贝技术
  ...
  // vector作为参数类型时，使用右值引用,右值引用可以避免拷贝构造函数的调用

  vector& operator=(vector&& rhs) noexcept;

  vector& operator=(std::initializer_list<value_type> ilist)
  {
    vector tmp(ilist.begin(), ilist.end()); // 用初始化列表构造一个临时对象
    swap(tmp); // 交换tmp和*this,这样*this的内存就可以被回收,目的是为了减少内存的使用
    return *this; // 返回*this
  }

  ~vector()
  { 
    // cap_ - begin_表示已经使用的空间
...
public:
  ...
  // static_cast<size_type>(-1)/sizeof(T)表示最大的size
...
fill_init(size_type n, const value_type& value)
{
  ...
  init_space(n, init_size); //分配n个空间，但是不初始化
  // 可用空间大于等于 n
